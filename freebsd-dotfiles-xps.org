...
# status placeholder
status-placeholder "r:reload  R:reload-all d:read o:open-in-browser y:copy-url n:exec-sauron  i:exec-imv  q:quit"

# sauron
bind n exec sauron "%l" 1>/dev/null

# open image links with imv
bind i exec imv "%l"

# page centering false
pager-centering false
...
#+NAME: newsraft-home-dir
#+BEGIN_SRC conf :tangle "~/.config/newsraft/config"
<<newsraft>>
#+END_SRC

**** current dir

#+NAME: newsraft-current-dir
#+BEGIN_SRC conf :tangle ".config/newsraft/config"
<<newsraft>>
#+END_SRC
...
# compose.yaml
COMPOSE_FILE  	:= ${HOME}/podman/freebsd-nixpkgs/containers/web/searxng/compose.yaml
COMPOSE_CMD	:= doas podman-compose -f
ENV_FILE  	:= ${HOME}/podman/freebsd-nixpkgs/containers/web/searxng/.env

JAIL_NAME	:= searxng-nix
STARTUP_SCRIPT	:= searxng-run
...
container: 
	wrapper-freebsd-nix -j $(JAIL_NAME) -a $(STARTUP_SCRIPT) -c $(COMPOSE_FILE) -e $(ENV_FILE) -p off

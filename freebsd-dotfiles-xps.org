...
menu: 
  "w":
    desc: general
    submenu:
      "m": { desc: mpv play, cmd: mpv "$(wl-paste)" 1>/dev/null }
  "o":
    desc: obs
    submenu:
...
max_render_time = -1
plugins = autostart command expo grid idle move oswitch place resize switcher vswitch window-rules vswipe extra-gestures wm-actions follow-focus wayfire-shell
